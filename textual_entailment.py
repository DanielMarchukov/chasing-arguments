...
class TextualEntailment:
    ...
    def run_predict(self, s1, s2):
        evi_sentence = [self.__preproc.fit_to_size(np.vstack(self.__preproc.sent_to_seq(s1)[0]),
                                                   (self.__preproc.get_evidence_length(),
                                                    self.__preproc.get_vector_size()))]

        hyp_sentence = [self.__preproc.fit_to_size(np.vstack(self.__preproc.sent_to_seq(s2)[0]),
