...
class LSTM:
    def __init__(self, e_length, h_length, v_size):
        ...
        self.__hyp = tf.placeholder(tf.float32,
                                    [self.__batch_size, self.__max_hypothesis_length, self.__vector_size],
                                    'hypothesis')
        self.__evi = tf.placeholder(tf.float32,
                                    [self.__batch_size, self.__max_evidence_length, self.__vector_size],
                                    'evidence')
        self.__y = tf.placeholder(tf.float32,
                                  [self.__batch_size, self.__n_classes],
                                  'label')
        ...
        self.__fc_weight = tf.get_variable('fc_weight',
                                           [2 * self.__hidden_size, self.__n_classes],
        ...
        with tf.variable_scope("loss"):
            ...
            self.__total_loss = self.__loss + self.__weight_decay * tf.add_n(
        ...
        with tf.device("/device:GPU:0"):
            prediction = self.__sess.run(self.__classification_scores,
                                         feed_dict={self.__hyp: (evi_sentence * self.__batch_size),
                                                    ...
                                                    self.__y: [[0, 0, 0]] * self.__batch_size})
